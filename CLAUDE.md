...
- Backend (dev):
  ...
  - Run from INSIDE backend/. Modules use flat imports (`from config import settings`),
    so `uvicorn backend.main:app` from the repo root fails with ModuleNotFoundError.
  - --host 0.0.0.0 is required for a physical device to reach it.
- Tests & lint:
...
- Backend (FastAPI):
  ...
  - Key API endpoints: `POST /api/summary` (see `backend/api/summary.py`), plus
    `POST /api/chat` and `POST /api/analyze` (see `backend/api/chat.py`).
  - Prompts are built server-side in `backend/services/prompts.py`. The app posts
    precomputed statistics and holds NO API key: `EXPO_PUBLIC_*` values are inlined
    into the JS bundle at build time and ship inside the app.
...
- When creating a new function, always include a one-sentance description of what it does
