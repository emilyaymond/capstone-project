...
import { getMetricConfig, getMetricChartKind, getMetricAggregation } from "./metricConfig";
...
function aggregateData(
  ...
  data.forEach((m) => {
    const k = Math.floor(new Date(m.timestamp).getTime() / bucketSize) * bucketSize;
...
function RangeBar({ range, color }: { range: "normal" | "warning" | "danger" | undefined; color: string }) {
  if (!range) return null;
  const barColor = range === "danger" ? "#FF3B30" : range === "warning" ? "#FF9500" : color;
  return <View style={[styles.rangeBar, { backgroundColor: barColor }]} />;
}

/** Stat pill used in the key-stats grid */
function StatPill({
  label,
  value,
  unit,
  ...
  return (
    ...
    >
      ...
      <ThemedText style={[styles.statPillValue, highlight && color ? { color } : undefined]}>
        {value}
      </ThemedText>
      <ThemedText style={styles.statPillUnit}>{unit}</ThemedText>
    </View>
  );
}
...
export default function MetricDetailScreen() {
  ...
  const touchTargetSize =
    mode === "simplified" ? TOUCH_TARGET_SIZES.simplified : TOUCH_TARGET_SIZES.minimum;
  ...
  useEffect(() => {
    announceNavigation(`${cfg.label} detail`, cfg.normalRange ? `Normal range: ${cfg.normalRange}` : undefined);
  ...
  const data: HealthMetric[] = useMemo(() => {
    ...
    switch (timeRange) {
      case "D":  bucketSize = 5 * 60 * 1000; break;        // 5-min
      case "W":  bucketSize = 60 * 60 * 1000; break;       // 1-hour
      case "M":  bucketSize = 6 * 60 * 60 * 1000; break;   // 6-hour
      case "6M": bucketSize = 24 * 60 * 60 * 1000; break;  // daily
      case "Y":  bucketSize = 7 * 24 * 60 * 60 * 1000; break; // weekly
      default:   bucketSize = 5 * 60 * 1000;
    }
  ...
  const heroValue = useMemo<string>(() => {
    ...
    if (isSleep) {
      ...
      const sleepData = allRaw.filter((m) =>
        new Date(m.timestamp).getTime() >= getStartDate(timeRange).getTime()
    ...
    if (agg === "sum") {
      return formatValue(vals.reduce((a, b) => a + b, 0), unit);
    }
    if (agg === "latest") {
  ...
  const heroLabel = useMemo<string>(() => {
    if (isLongRange(timeRange) && cfg.longRangeHeroLabel) return cfg.longRangeHeroLabel;
    if (isSleep) return timeRange === "D" ? "Time Asleep" : cfg.longRangeHeroLabel ?? "Avg / night";
  ...
  const keyStats = useMemo(() => {
    ...
    const outlierCount = vals.filter((v) => Math.abs(v - avg) > 2 * stdDev).length;
    const unit = data[0]?.unit ?? cfg.unit;
  ...
  const handleRangeChange = (range: TimeRange) => {
    setTimeRange(range);
    announceDataUpdate(`Showing ${rangeSubtitleText(range).toLowerCase()} data`);
  };
  ...
  const heroA11yLabel = useMemo(() => {
    ...
    if (keyStats?.outlierCount) return `${base} ${keyStats.outlierCount} outlier${keyStats.outlierCount > 1 ? "s" : ""} detected.`;
    return base;
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.topRow}>
          <AccessibleButton
            ...
            onPress={() => router.back()}
            variant="secondary"
            style={[styles.circleButton, { minWidth: touchTargetSize, minHeight: touchTargetSize }]}
          >
        ...
        {/* ── Hero stat block ────────────────────────────────────────── */}
        <View
          style={[styles.heroCard, { borderLeftColor: cfg.color, borderLeftWidth: 4 }]}
          accessible
          accessibilityRole="summary"
          accessibilityLabel={heroA11yLabel}
        >
          <RangeBar range={latest?.range} color={cfg.color} />
          ...
          {!isSleep && data[0]?.unit && (
            <ThemedText style={[styles.heroUnit, { fontSize: fontSize.body }]}>
              {data[0].unit}
            </ThemedText>
          )}
          <ThemedText style={[styles.heroSub, { fontSize: fontSize.label }]}>
            {rangeSubtitle}
            {cfg.normalRange ? `  ·  Normal: ${cfg.normalRange}` : ""}
          </ThemedText>

          {/* Range status badge */}
          {latest?.range && latest.range !== "normal" && (
            <View
              style={[
                styles.rangeBadge,
                { backgroundColor: latest.range === "danger" ? "#FFE5E5" : "#FFF3E0" },
              ]}
              accessible
              accessibilityLabel={latest.range === "danger" ? "Value outside normal range" : "Value slightly elevated"}
            >
              <ThemedText
                style={[
        ...
        >
          {isSleep ? (
            data.length ? (
              ...
              <ThemedText style={styles.emptyChart}>No sleep data for this period.</ThemedText>
            )
          ) : points.length ? (
            chartKind === "scatter" ? (
              <ScatterPlot
            ...
            <ThemedText style={styles.emptyChart}>No {cfg.label.toLowerCase()} data for this period.</ThemedText>
          )}
        </View>
        ...
        {latest && (
          ...
          >
            <ThemedText style={[styles.latestLeft, { fontSize: fontSize.label }]}>
              Latest · {formatTimestamp(latest.timestamp, timeRange)}
            </ThemedText>
            <ThemedText style={[styles.latestRight, { fontSize: fontSize.label, color: cfg.color }]}>
              {formatValue(Number(latest.value), latest.unit ?? cfg.unit)}
              {latest.unit && !["hr"].includes(latest.unit) ? ` ${latest.unit}` : ""}
            </ThemedText>
          </View>
        )}
        ...
        {isSleep && (
          <>
            ...
            >
              <View style={styles.sleepHeroRow}>
                <View style={styles.sleepHeroItem}>
                  <ThemedText style={styles.sleepHeroItemLabel}>In Bed</ThemedText>
                  <ThemedText style={styles.sleepHeroItemValue}>
                    {sleepBreakdown ? formatSleepDuration(sleepBreakdown.totalInBed ?? sleepBreakdown.totalSleep) : "—"}
                  </ThemedText>
                </View>
                <View style={styles.sleepHeroDivider} />
                <View style={styles.sleepHeroItem}>
                  <ThemedText style={styles.sleepHeroItemLabel}>Efficiency</ThemedText>
                  <ThemedText style={styles.sleepHeroItemValue}>
                ...
                <View style={styles.sleepHeroDivider} />
                <View style={styles.sleepHeroItem}>
                  <ThemedText style={styles.sleepHeroItemLabel}>Sessions</ThemedText>
                  <ThemedText style={styles.sleepHeroItemValue}>{data.length}</ThemedText>
                </View>
              </View>
            </View>
            ...
            <View
              ...
              accessible
              accessibilityRole="summary"
              accessibilityLabel={`Sleep stage breakdown for ${rangeSubtitle}`}
            >
              <ThemedText style={[styles.cardTitle, { fontSize: fontSize.body + 2 }]}>
                Stage Breakdown
              </ThemedText>
              {data.length > 0 ? (
                <SleepStageBreakdown sleepMetrics={data} />
              ) : (
                <ThemedText style={styles.emptyChart}>No stage data available.</ThemedText>
              )}
            </View>
            ...
            >
              ...
              <View style={styles.statsGrid}>
                {[
                  { label: "Total Sleep", val: sleepBreakdown ? formatSleepDuration(sleepBreakdown.totalSleep) : "—" },
                  { label: "Deep Sleep", val: sleepBreakdown?.deepSleep != null ? formatSleepDuration(sleepBreakdown.deepSleep) : "—" },
                  { label: "Light Sleep", val: sleepBreakdown?.lightSleep != null ? formatSleepDuration(sleepBreakdown.lightSleep) : "—" },
                  { label: "REM", val: sleepBreakdown?.remSleep != null ? formatSleepDuration(sleepBreakdown.remSleep) : "—" },
                ].map(({ label, val }) => (
                  <View key={label} style={styles.statItem} accessible accessibilityLabel={`${label}: ${val}`}>
                    <ThemedText style={styles.statLabel}>{label}</ThemedText>
                    <ThemedText style={[styles.statValue, { color: cfg.color }]}>{val}</ThemedText>
                  </View>
                ))}
              </View>
            </View>
...
const styles = StyleSheet.create({
  ...
  circleButtonText: { fontSize: 22, fontWeight: "600", color: "#007AFF" },
  titleBlock: { flex: 1, alignItems: "center" },
  screenTitle: { fontWeight: "800", textAlign: "center" },
  ...
  statPillLabel: { fontSize: 11, color: "#8E8E93", fontWeight: "700", textTransform: "uppercase", letterSpacing: 0.4 },
  statPillValue: { fontSize: 16, fontWeight: "800", color: "#1C1C1E" },
  statPillUnit: { fontSize: 10, color: "#8E8E93" },
  ...
  sleepHeroItemLabel: { fontSize: 12, color: "#5856D6", fontWeight: "700", textTransform: "uppercase", letterSpacing: 0.5 },
  sleepHeroItemValue: { fontSize: 20, fontWeight: "800", color: "#1C1C1E" },
  sleepHeroDivider: { width: 1, height: 36, backgroundColor: "rgba(88,86,214,0.2)" },

  // Stats grid (sleep insights)
  statsGrid: { flexDirection: "row", flexWrap: "wrap", gap: 12 },
  statItem: { width: "46%", gap: 4 },
  statLabel: { fontSize: 12, color: "#8E8E93", fontWeight: "600", textTransform: "uppercase", letterSpacing: 0.4 },
