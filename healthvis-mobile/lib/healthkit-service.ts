...
const DEFAULT_RETRY_OPTIONS: RetryOptions = {
  ...
  shouldRetry: (error: any, attempt: number) => {
    ...
    if (isHealthKitError(error)) {
      return (
        error.code !== HealthKitErrorCode.PERMISSION_DENIED &&
  ...
  onRetry: (error: any, attempt: number, delay: number) => {
    console.log(
      `Retrying HealthKit operation (attempt ${attempt}) after ${delay}ms:`,
      error,
    );
  },
};
...
export function convertHealthKitSample(
  ...
  const rawValue =
    typeof sample.value === "number"
      ? sample.value
      : parseFloat(String(sample.value));
...
export const healthKitService: HealthKitService = {
  ...
  async initializeHealthKit(): Promise<PermissionStatus> {
    try {
        ...
        return new Promise<PermissionStatus>((resolve, reject) => {
          ...
          AppleHealthKit.initHealthKit(permissions, (error: string) => {
            if (error) {
              ...
              reject(
                HealthKitError.initializationFailed(error, {
                  operation: "initializeHealthKit",
                }),
              );
              return;
            ...
            healthKitService.checkPermissions().then(resolve).catch(reject);
          });
        });
      ...
      if (
        healthKitError.code === HealthKitErrorCode.NOT_AVAILABLE ||
  ...
  async checkPermissions(): Promise<PermissionStatus> {
    ...
    try {
      ...
      const [steps, distance, flightsClimbed, activeEnergy, exerciseMinutes] =
        await Promise.all([
      ...
      const [weight, height, bmi, bodyFatPercentage] = await Promise.all([
      ...
      const [water, protein, carbohydrates, sleep] = await Promise.all([
  ...
  async fetchWithErrorHandling(
    ...
    try {
      ...
      const result = await retryWithBackoff(operation, DEFAULT_RETRY_OPTIONS);
      ...
      if (
        healthKitError.code === HealthKitErrorCode.NO_DATA ||
      ...
      console.error(
        `Critical error in ${operationName}:`,
        healthKitError.toJSON(),
      );
      return [];
    }
  },
          ...
          AppleHealthKit.getHeartRateSamples(
            ...
            (err: string, results: HealthValue[]) => {
              ...
              try {
                ...
                reject(
                  HealthKitError.conversionFailed(
                    "fetchHeartRate",
                    conversionError,
                  ),
                );
  ...
  async fetchDietaryEnergy(options: FetchOptions): Promise<HealthMetric[]> {
    return new Promise((resolve, reject) => {
      ...
      if (
        typeof (AppleHealthKit as any).getEnergyConsumedSamples === "function"
      ) {
        (AppleHealthKit as any).getEnergyConsumedSamples(
          healthKitOptions,
          (err: string, results: HealthValue[]) => {
            if (err) {
        ...
        console.warn(
          "getEnergyConsumedSamples not available in react-native-health",
        );
        resolve([]);
      }
    });
  ...
  async fetchFats(options: FetchOptions): Promise<HealthMetric[]> {
    return new Promise((resolve, reject) => {
      ...
      if (typeof (AppleHealthKit as any).getFatTotalSamples === "function") {
        (AppleHealthKit as any).getFatTotalSamples(
          healthKitOptions,
          (err: string, results: HealthValue[]) => {
            if (err) {
   ...
   *
   * Sleep analysis includes different sleep stages:
   * - ASLEEP_CORE: Light or intermediate sleep
   * - ASLEEP_DEEP: Deep sleep
   * - ASLEEP_REM: REM sleep
   * - ASLEEP_UNSPECIFIED: Asleep, but stage unknown
   * - AWAKE: User is awake
        ...
        (err: string, results: any[]) => {
          ...
          const metrics = results.map((sample, index) => {
            ...
            healthValue.metadata = {
              ...sample.metadata,
              sleepStage: sleepStage, // Human-readable sleep stage
              rawSleepStage: rawStage, // Original Apple value
              originalValue: sample.value, // Keep original categorical value
              durationMinutes: Math.round(durationMs / (1000 * 60)), // Duration in minutes
            };
  ...
  async fetchMindfulness(options: FetchOptions): Promise<HealthMetric[]> {
    return new Promise((resolve, reject) => {
      ...
      if (typeof (AppleHealthKit as any).getMindfulSession === "function") {
        (AppleHealthKit as any).getMindfulSession(
          healthKitOptions,
          (err: string, results: any[]) => {
            if (err) {
  ...
  async fetchAllBody(options: FetchOptions): Promise<HealthMetric[]> {
    try {
      ...
      // Combine all metrics into a single array
      const allMetrics = [...weight, ...height, ...bmi, ...bodyFatPercentage];
  ...
  async fetchAllNutrition(options: FetchOptions): Promise<HealthMetric[]> {
    try {
      ...
      const [dietaryEnergy, water, protein, carbohydrates, fats] =
        await Promise.all([
    ...
    try {
      ...
      results.forEach((result, index) => {
        if (result.status === "fulfilled") {
          ...
          console.error(
            `Failed to fetch ${categories[index]} data:`,
            result.reason,
          );
        }
      });
