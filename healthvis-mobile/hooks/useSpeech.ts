...
export function useSpeech(): UseSpeechReturn {
   ...
   */
  const speakHealthMetricSummary = useCallback(
    async (
      metrics: HealthMetric[],
      category?: HealthCategory,
    ): Promise<void> => {
      try {
        ...
        for (const metric of metricsByType.values()) {
          ...
          const rangeDescription = metric.range
   ...
   */
  const speakHealthMetricDetails = useCallback(
    async (metric: HealthMetric): Promise<void> => {
      try {
        const metricName = getDisplayNameForType(metric.type);
   ...
   */
  const speakCategorySummary = useCallback(
    async (
      category: HealthCategory,
      metrics: HealthMetric[],
    ): Promise<void> => {
      try {
        if (!metrics || metrics.length === 0) {
          await speak(
            `No ${formatCategory(category).toLowerCase()} data available.`,
          );
          return;
        }
